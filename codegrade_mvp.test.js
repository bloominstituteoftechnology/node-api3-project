...
describe('server.js', () => {
  ...
  describe('[GET] /api/users/:id', () => {
    test('[3] can get the correct user', async () => {
      let res = await request(server)
        .get('/api/users/1')
      expect(res.body)
        .toMatchObject(initialUsers[ 0 ])
      expect(res.body)
        .toHaveProperty('id')
      res = await request(server)
        .get('/api/users/2')
      expect(res.body)
        .toMatchObject(initialUsers[ 1 ])
      expect(res.body)
        .toHaveProperty('id')
    }, 500)
  ...
  describe('[POST] /api/users', () => {
    test('[6] creates a new user in the database', async () => {
      ...
      expect(users)
        .toHaveLength(initialUsers.length + 2)
    }, 500)
    test('[7] responds with the newly created user', async () => {
      let res = await request(server)
        ...
        .send({ name: 'foo' })
      expect(res.body)
        .toMatchObject({ id: 10, name: 'foo' })
      res = await request(server)
        .post('/api/users')
        .send({ name: 'bar' })
      expect(res.body)
        .toMatchObject({ id: 11, name: 'bar' })
    }, 500)
    ...
    test('[9] responds with the correct error message if missing name', async () => {
      let res = await request(server)
        ...
        .send({ random: 'thing' })
      expect(res.body.message)
        .toMatch(/missing required name/i)
    }, 500)
  })
  describe('[PUT] /api/users/:id', () => {
    test('[10] writes the updates in the database', async () => {
      ...
      let users = await db('users')
      expect(users[ 0 ])
        .toMatchObject({ id: 1, name: 'FRODO BAGGINS' })
    }, 500)
  ...
  describe('[DELETE] /api/users/:id', () => {
    test('[15] deletes the user from the database', async () => {
      ...
      let users = await db('users')
      expect(users[ 0 ])
        .toMatchObject({ name: 'Samwise Gamgee' })
    }, 500)
  ...
  describe('[GET] /api/users/:id/posts', () => {
    test('[18] gets the correct number of user posts', async () => {
      ...
      expect(res.body)
        .toHaveLength(initialPosts.filter(p => p.user_id == 1).length)
    }, 500)
    test('[19] responds with a 404 if user id does not exist', async () => {
      const res = await request(server)
        .get('/api/users/111/posts')
      expect(res.status)
        .toBe(404)
    }, 500)
  })
  describe('[POST] /api/users/:id/posts', () => {
    test('[20] creates a new user post in the database', async () => {
      ...
      let posts = await db('posts')
        .where('user_id', 1)
      expect(posts)
        .toHaveLength(initialPosts.filter(p => p.user_id == 1).length + 1)
      await request(server)
        .post('/api/users/1/posts')
        .send({ text: 'bar' })
      posts = await db('posts')
        .where('user_id', 1)
      expect(posts)
        .toHaveLength(initialPosts.filter(p => p.user_id == 1).length + 2)
    }, 500)
    ...
    test('[24] responds with the correct error message if missing text', async () => {
      let res = await request(server)
        ...
        .send({ no: 'foo' })
      expect(res.body.message)
        .toMatch(/missing required text/i)
    }, 500)
  })
})
